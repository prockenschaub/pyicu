...
def test_validate_errors(example_df):
    ...
    with pytest.raises(AttributeError) as e_info:
        example_df.set_index(["index_var", "id_var", "dur_var"]).tbl._validate()
    assert e_info.match(".*the second and third must be a time index.*")
    with pytest.raises(AttributeError) as e_info:
        example_df.set_index(["id_var", "index_var", "dur_var", "val_var"]).tbl._validate()
    ...
    example_df.set_index(["id_var", "index_var", "dur_var"]).tbl._validate()
...
def test_win_tbl_as_id_tbl(win_tbl):
    id_tbl1 = win_tbl.tbl.as_id_tbl(id_var="val_var")
    assert id_tbl1.tbl.is_id_tbl()
    assert id_tbl1.tbl.id_var == "val_var"
    assert all([i in id_tbl1.columns for i in win_tbl.index.names])
    id_tbl2 = win_tbl.tbl.as_id_tbl()
    assert id_tbl2.tbl.is_id_tbl()
    assert id_tbl2.tbl.id_var == "id_var"
...
def test_ts_tbl_as_ts_tbl(ts_tbl):
    ...
    ts_tbl3 = ts_tbl.tbl.as_ts_tbl(id_var="val_var", index_var="dur_var")
    assert ts_tbl3.tbl.id_var == "val_var" and ts_tbl3.tbl.index_var == "dur_var"


def test_win_tbl_as_ts_tbl(win_tbl):
    ts_tbl1 = win_tbl.tbl.as_ts_tbl()
    assert ts_tbl1.tbl.is_ts_tbl()
    assert ts_tbl1.tbl.id_var == "id_var" and ts_tbl1.tbl.index_var == "index_var"


def test_set_dur(ts_tbl):
    with pytest.raises(ValueError) as e_info:
        ts_tbl.tbl.set_dur_var("unknown_var")
...
def test_is_win_tbl(example_df, id_tbl, ts_tbl, win_tbl):
    assert example_df.tbl.is_win_tbl() == False
    assert id_tbl.tbl.is_win_tbl() == False
    assert ts_tbl.tbl.is_win_tbl() == False
    assert win_tbl.tbl.is_win_tbl() == True


def test_pandas_as_win_tbl(example_df):
    def assert_vars(x, id, index, dur):
        assert x.tbl.id_var == id
        assert x.tbl.index_var == index
        assert x.tbl.dur_var == dur
    
    win_tbl1 = example_df.tbl.as_win_tbl(id_var="id_var", index_var="index_var", dur_var="dur_var")
    assert_vars(win_tbl1, "id_var", "index_var", "dur_var")
    win_tbl2 = example_df.tbl.as_win_tbl(id_var="id_var")
    assert_vars(win_tbl2, "id_var", "index_var", "dur_var")
    win_tbl3 = example_df[["id_var", "val_var", "dur_var", "index_var"]].tbl.as_win_tbl(id_var="id_var")
    assert_vars(win_tbl3, "id_var", "dur_var", "index_var")
    with pytest.raises(TypeError) as e_info:
        example_df[["id_var", "index_var", "val_var"]].tbl.as_win_tbl()
    assert e_info.match(".*no suitable time column could be found.*")


def test_id_tbl_as_win_tbl(id_tbl):
    win_tbl1 = id_tbl.tbl.as_win_tbl(dur_var="dur_var")
    assert win_tbl1.tbl.is_win_tbl()
    assert win_tbl1.tbl.dur_var == "dur_var"
    win_tbl2 = id_tbl.tbl.as_win_tbl(index_var="dur_var")
    assert win_tbl2.tbl.dur_var == "index_var"


def test_ts_tbl_as_win_tbl(ts_tbl):
    win_tbl1 = ts_tbl.tbl.as_win_tbl(dur_var="dur_var")
    assert win_tbl1.tbl.is_win_tbl()
    assert win_tbl1.tbl.dur_var == "dur_var"
    win_tbl2 = ts_tbl.tbl.as_win_tbl()
    assert win_tbl2.tbl.is_win_tbl()
    assert win_tbl2.tbl.dur_var == "dur_var"


def test_win_tbl_as_win_tbl(win_tbl):
    win_tbl1 = win_tbl.tbl.as_win_tbl()
    assert win_tbl1.tbl.is_win_tbl()
    assert win_tbl1.tbl.dur_var == "dur_var"
    win_tbl['new_dur_var'] = win_tbl.index.get_level_values(2)
    win_tbl2 = win_tbl.tbl.as_win_tbl(dur_var="new_dur_var")
    assert win_tbl2.tbl.is_win_tbl()
    assert win_tbl2.tbl.dur_var == "new_dur_var"
